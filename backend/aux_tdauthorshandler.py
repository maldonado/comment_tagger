...
def search_authors(repository_id, repository_name):
    ...
    connection = PSQLConnection.get_connection()
    cursor = connection.cursor() 
    cursor.execute("select a.file_id, a.treated_comment_text from processed_comments a, technical_debt_summary b, processed_comment c where  b.processed_comment_id = c.id and c.treated_commenttext = a.treated_comment_text and a.repository_id = %s  group by 1,2 order by 1   ", (repository_id, ))
